...
[options="header"]
|===
|Deployment method |Dynamic scale |Recommended use casefootnote:[This is largely based on our experiences and may be biased to large companies and academic environments. Please feel free to make your own call.] |Dependency management |Notebook deployed insidefootnote:[There are https://oreil.ly/TqhBb[some workarounds].] |Mixed worker types
...
In 2020 Dask added a https://oreil.ly/otOQX[+++DaskHub Helm chart+++], which combines the deployment of JupyterHub with the Dask Gateway.

=== Dask on Ray

Deploying Dask on Ray is a little bit different than all of the other options, in that it changes not only how Dask workers and tasks are scheduled, but also changes https://oreil.ly/PIwuX[+++how Dask objects are stored+++]. This can reduce the number of copies of the same object that need to be stored, allowing you to use your cluster memory more efficiently.
...
However, if you don't have an existing Ray cluster, you will still need to deploy Ray somewhere with the same considerations as Dask. Deploying Ray is beyond the scope of this book. Ray's https://oreil.ly/PNb0_[+++production guide+++] has details for deploying on Ray, as does _Scaling Python with Ray_.
...
.A successfully deployed Dask instance on JupyterLab
...
Dask jobs tend to take a long time to run, since otherwise we would not be putting in the effort to parallelize them. You can use Dask's `progress` function from pass:[<code>dask&#x200b;.dis&#x2060;tributed</code>] to track your futures' progress inside your notebook itself (see <<dask_progress_monitoring_ch12_1685475108548>>).
...
.A task stream with well-balanced workers
...
.A task stream with too many small data chunks
...
.Memory usage by worker in the monitoring UI
...
.Progress monitoring by task, summed over all workers
...
.A task graph showing the color-coded status of each task and its preceding and succeeding tasks
