...
The JupyterLab extension links to the Dask web UI, and you can also get a link through the clusters repr. If the cluster link does not work/is not accessible, you can try installing the `jupyter-server-proxy` extension so you can use the notebook host as a https://en.wikipedia.org/wiki/Jump_server[+++jump host+++].
