...
YARN is a popular cluster manager from ((("deployment", "on YARN", id="dpyyrn")))((("YARN", "Dask deployment", id="YRNdkpy")))((("CMs (cluster managers)", "YARN", id="cmyrn")))the big data world that is available in open source as well as commercial on-premises (e.g., Cloudera) and cloud (e.g., Elastic Map Reduce). There are several ways to run Dask on a YARN cluster: one is with Dask-Yarn and ((("Dask-Yarn")))((("Dask-Gateway")))the other is with Dask-Gateway. While the two methods are similar, Dask-Gateway is potentially a bit more involved, as it adds a centrally managed server that runs to manage Dask clusters, but it has more fine-grained security and administrative controls. 
...
Dask has gained a big academic and scientific ((("deployment", "on HPC (high-performance computing)")))((("high-performance computing", see="HPC (high-performance computing)")))user base. Part of this comes from how you can use existing high-performance computing (HPC) clusters with Dask to make readily available scalable scientific computing without rewriting all of your code.footnote:[In some ways, HPC and cluster managers are different names for the same thing, with cluster managers coming out of industry and HPC out of research. HPC clusters tend to have and use a shared network storage that is not as common in industry.]
...
Part of running Dask remotely is being ((("deployment", "on HPC (high-performance computing)", "local machines", id="dyhgpf")))((("HPC (high-performance computing)", "local machines", id="hpclmc")))able to connect to the server to run your tasks. If you want to connect your client to a remote cluster, run Jupyter remotely, or just access the UI on a cluster, you'll need to be able to connect to some ports on the remote machine.
...
Dask jobs tend to take a long time to run, since ((("deployment", "Jupyter", "progress tracking")))((("Jupyter", "deployment on", "progress tracking")))otherwise we would not be putting in the effort to parallelize them. You can use Dask's `progress` function from pass:[<code>dask&#x200b;.dis&#x2060;tributed</code>] to track your futures' progress inside your notebook itself (see [.keep-together]#<<dask_progress_monitoring_ch12_1685475108548>>).#
...
Dask's dashboard contains many different pages, each of which can help with understanding different parts of ((("performance", "metrics", id="prfrmtr")))((("metrics", "distributed computing", id="mtdbcp")))((("distributed systems", "metrics", id="dsbym")))your program.
...
Auto/adaptive scaling takes ((("clusters", "sizing")))care of the question of "how many" but not "how big" each worker should be. That said, here are some general rules of thumb:
...
Like many other distributed ((("deployment", "monitoring", id="dpymtr")))((("monitoring", "deployment", id="mtrdpy")))libraries, Dask provides logs, and you can configure Dask logs to be sent to a storage system. The method will vary by the deployment environment and if you are using Jupyter.
