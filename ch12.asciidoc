...
You can monitor Dask real-time with the dashboard, but it will disappear once you close out your cluster. You can save the html page, as well as export the metrics as a DataFrame itself, and write out custom code for metrics (see <<ex_generate_performance_report>>).
...
In <<appA>>, you will learn details about testing and validation, which is especially important for scheduled and automated jobs as there is no time for manual checking.
