...
One generic way you can get the worker and scheduler logs is through `get_worker_logs()` and `get_scheduler_logs()` on the Dask client. You can specify specific topics and log to/read from just the relevant topics. <<ex_basic_logging_ch12_1685536257105>> shows how to log and read from custom topics.
