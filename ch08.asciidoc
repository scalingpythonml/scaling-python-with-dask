...
Still, there are already dozens of popular libraries specific for Dask, and the open-source community around Dask is growing around them, that gives us some confidence that many of these are here to stay. We prepared a non-exhaustive list of foundational libraries in use, and its relation to the core Dask project. The following table is meant as a roadmap for the users, not our endorsement of individual repository. For the libraries we cover in this book, we offer some of our own evaluation of those libraries as well.
