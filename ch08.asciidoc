...
One of the simplest metrics to check is if people are using a package, which you can see by looking at the installs. You can check PyPi package installs stats on https://oreil.ly/1HHL8[+++pypistats+++] (see <<dask-kubernetes-install-stats>>) or on https://oreil.ly/83RIO[+++Google's BigQuery+++], and conda installs using the https://oreil.ly/4STsP[+++condastats library+++].
