...
The majority of tools for scaling ((("scaling DataFrames", "distributed")))DataFrames are distributed in nature, since all of the fancy tricks on a single machine can only get you so far.

===== ASF Spark DataFrame

Spark started out with what they ((("scaling DataFrames", "Spark")))((("Spark DataFrames")))called a resilient distributed dataset (RDD) and then quickly added a more DataFrame-like API called DataFrames. This caused much excitement, but many folks interpreted it to mean "pandas-like," whereas Spark's (initial) DataFrames was more akin to "SQL-like" DataFrames. Spark is written primarily in Scala and Java, both of which run on the ((("JVM (Java Virtual Machine)")))Java Virtual Machine (JVM). While Spark has a Python API, it involves substantial data transfer between the JVM and Python, which can be slow and increase memory requirements. Spark DataFrames was created before ASF Arrow, and so it has its own in-memory storage format, but it has since added support for using Arrow for communication between the JVM and Python.
...
cuDF not only has CI, it also has a strong ((("scalable DataFrames", "tools", startref="sclbdftls")))culture of code review with per-area responsibilities.
