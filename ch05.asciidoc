...
This chapter focuses on two of Dask's often overlooked collections, arrays and bags.
While much of what you've learned in the previous chapter on DataFrames does carry forward to arrays and bags, there are some important differences that make each collection type better suited to specific classes of problems. As mentioned in the <<hello_worlds>> section, Dask bags implement common functional APIs and Dask Arrays implement a subset of numpy arrays.
