...
To avoid the slowness of having ((("libraries", "preinstalling")))to install packages each time a new worker is launched, you should try to preinstall your libraries. Each cluster ((("dependencies", "cluster managers")))((("CMs (cluster managers)", "dependencies")))manager (e.g., YARN, Kubernetes, Coiled, Saturn, etc.) has its own methods for managing dependencies. This can happen at runtime or at setup where the packages are preinstalled. The specifics for the different cluster managers are covered in <<ch12>>.
