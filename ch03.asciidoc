...
The Dask UI during the running of the examples of this chapter is shown in [.keep-together]#<<fig_dask_ui>>.#
...
You should try this out now by visualizing everyone's favorite word count example from the previous chapter. When you call `dask.visualize` on pass:[<code>words_bag&#x200b;.frequen&#x2060;cies()</code>], you should get a result that looks something like <<vis_ex>>.
...
Dask's local mode has a best-effort caching system based on cachey. Since this only works in local mode, we won't go into the details, but if you are running in local mode, you can take a look at https://oreil.ly/VFSVQ[the local cache documentation].
