...
To take the most advantage of Dask's graph ((("computation, combining")))((("graphs", "combining computation")))((("compute function")))((("functions", "compute")))optimization, it's important to submit your work in larger batches. First off, when you're blocking on the result with `dask.compute`, small batches can limit the parallelism. If you have a shared parent&mdash;say, two results on the same data&mdash;submitting the computations together allows Dask to share the computation of the underlying data. You can verify whether Dask is able to share a common node by calling `visualize` on your list of tasks (e.g., if you take pass:[Examples <a data-type="xref" data-xrefstyle="select:labelnumber" href="#custom_array_chunk_size">#custom_array_chunk_size</a> and <a data-type="xref" data-xrefstyle="select:labelnumber" href="#task_dependencies_example_ch03_1686946028901">#task_dependencies_example_ch03_1686946028901</a>] and visualize both the tasks together, you'll see the shared node in <<vis_ex>>).
