...
Dask futures offer a non-mutable ((("mutable state")))distributed state, where values are stored on the workers. However, this doesn't work well for situations where you want to update the state, like changing the value in a bank account balance (as we'll do in <<making_bank_acct_actor_ch07_1686239522527>>), or updating machine learning model weights during training.
...
It can be helpful to think of each actor as a person holding a note about their state, and they are the only ones allowed to read or update this note. Whenever another part of the code wants to access or modify the state, it must ask the actor to do this.
...
Now that you have an understanding of the ((("actor model", startref="actrmd")))actor model in general, it's time for you to learn about how Dask implements it, and its trade-offs.
...
As with `dask.delayed`, you can scale actors both horizontally (across pro&#x2060;cesses/&#x200b;machines) by creating multiple actors or vertically (with more resources). Scaling actors horizontally is not as simple as just adding more machines or workers, since Dask cannot break up a single actor across multiple processes.
...
We extend the bank account example to a "bank" where an actor may be responsible for multiple accounts (but not all of the accounts in the bank). We can then use an actor pool with hashing to route the requests to the correct "branch" or actor, as shown ((("actors", "scaling", startref="actrsc")))((("scaling", "actors", startref="sclgctr")))in <<sketchy_bank_ex>>.
...
[WARNING]
====
Calls to `dask.delayed` functions may be retried on failure, and if they call functions on actors, those function calls will then be duplicated. If you cannot have a function replayed, then you need to ensure it is only called from inside other actors.
====

Dask's actor model is less full-featured than Ray's actor model, much like Ray's DataFrame is less full-featured than Dask's. You may wish to consider running Dask on Ray to get the best of both worlds. While Holden is biased, she suggests you check out her book _Scaling Python with Ray_ if you are interested in Ray.
...
|*Scalability* |No, all state must fit on single machine. |State within each actor must fit on a machine, but actors are spread out. |Yesfootnote:[Ray actors still require that the state within an actor must fit on a single machine. Ray has additional tools to shard or create pools of actors.]
...
As with most things in life, picking the right technique is a compromise specific to the problem you are trying to solve. We believe that one of the two local (e.g., driver) states, or using Ray actors in conjunction with Dask for its analytics powers, can handle most cases where you need mutable state.
