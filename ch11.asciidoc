...
For categorical variables, while ((("Dask-ML", "Categorizer", id="dmlgz")))((("Categorizer", id="ctgzr")))there is `OneHotEncoder` in Dask-ML, it’s not as efficient and one-to-one in replacement as its scikit-learn equivalent. At this point we recommend using `Categorizer` to encode a categorical dtype.footnote:[For performance reasons&mdash;at the time of writing, Dask’s `OneHotEncoder` calls the `get_dummies` method from pandas, which is a slower implementation than scikit-learn’s `OneHotEncoder`. `Categorizer`, on the other hand, uses a Dask DataFrame aggregation method to scan through categories efficiently.]
...
Cross-validation and ((("Dask-ML", "hyperparameter tuning")))((("hyperparameter tuning, "Dask-ML")))hyperparameter tuning often is a costly process even with a smaller dataset, as anyone who has run the scikit-learn cross-validate would attest. Dask users often deal with datasets large enough that use of exhaustive search algorithms is not feasible. As an alternative, Dask-ML implements several additional adaptive algorithms and hyperband-based methods that approach the tuned parameter quicker with robust mathematical foundation.footnote:[Dask-ML’s own documentation has more info on adaptive and approximation CV methods implemented and use cases.] The ((("Dask-ML", "models", "selecting", startref="dmdlsc")))((("Dask-ML", "models", "training", startref="dkmdl")))authors of the `HyperbandSearchCV` methods do ask that the use be cited.footnote:[They note in the documentation to cite the following paper if using this method: S. Sievert, T. Augspurger, and M. Rocklin, “Better and Faster Hyperparameter Optimization with Dask,” _Proceedings of the 18th Python in Science Conference_ (2019), _https://doi.org/10.25080/Majora-7ddc1dd1-011_.]
...
XGBoost uses a DMatrix (data matrix) as its ((("DMatrix, XGBoost")))standard data format it works with. XGBoost has a built-in Dask compatible DMatrix, which takes in Dask array and Dask DataFrame. Once the Dask environment is set up, the use of gradient booster is as you would expect. Specify the learning rate, threads, and objective functions, as usual. <<ex_xgb_basic_usage>> works with a Dask CUDA cluster, and runs a standard gradient booster training.
...
A much newer addition is another library, Dask-SQL, which provides a convenient wrapper around simple ML model ((("ML (machine learning)", "Dask-SQL", id="mldsql")))((("Dask-SQL", "ML (machine learning) models", id="dskqmlm")))training workloads. <<Dask_sql_define_tables>> loads the same NYC yellow taxi cab data as a Dask DataFrame, then registers the view to Dask-SQL context.
