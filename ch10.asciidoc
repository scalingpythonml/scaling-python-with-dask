...
If you have decided that your problem is well suited to a specialized resource, the next step is to https://distributed.dask.org/en/latest/resources.html[make the scheduler aware of which machines and processes have the resource]. You can do this by adding either environment variable or command line flag to the worker launch (e.g. `--resources "GPU=2"` or `DASK_DISTRIBUTED__WORKER__RESOURCES__GPU=2`).
...
The three main CUDA libraries extending Dask are cudf (previously called dask-cudf), BlazingSQL, and cuML.footnote:[BlazingSQL may be end-of-life; there has not been a commit for an extended period of time and the website is just a hard-hat like those 1990s geocities websites.] Currently these libraries are focused on NVidia GPUs.
