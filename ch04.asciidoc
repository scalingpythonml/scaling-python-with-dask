...
Sometimes it's not possible to load the data with the right number of partitions, and in those cases you can repartition your data after the load.
...
You can use Dask's built-in support for loading SQL databases using SQLAlchemy (see <<ex_read_SQL_Dataframe>>). For Dask to split up the query on multiple machines, you need to give it an index key. Often SQL databases will have a primary key or numerical index key that you can use for this purpose (e.g., `read_sql_table("customers", index_col="customer_id")`). 
...
[TIP]
====
Dask only implements filtered reads and writes for Parquet. If you are reading data in another partitioned format, you can ignore the filtering or manually compute the required files.footnote:[You can also use a metastore, we'll discuss later.] There is no easy way to implement partitioned writes for other formats.
====
...
Dask does not support "streaming" nonrandom access input formats. This means that the data inside each file must fit entirely in memory.
****
...
Indexing is frequently used to filter the data to only have the components you need. I did this for the San Francisco COVID-19 data by looking at just the case rates for people of all vaccine statuses, as shown in <<index_covid_data>>.
...
Aggregations are another special case that can reduce the amount of data that needs to be transferred over the network. Aggregations are functions that combine records.
...
Therefore, if you need to compute the average and the max, you should compute both at the same time (<<max_mean>>).
...
For distributed systems like Dask, if an aggregation can be partially evaluated and then merged, you can potentially combine some records preshuffle. Not all partial aggregations are created equal. What matters with partial aggregations is that the amount of data is reduced when merging values with the same key compared to the original multiple values.
...
In some cases, like a pure summation, you don't need to do any postprocessing on `agg`&rsquo;s output, so you can skip the `finalize` function.
...
[WARNING]
====
Dask's set_index has a similar `partition_size` parameter, but as of this writing, https://github.com/dask/dask/issues/7110[only works to reduce the number of partitions].
====
...
You aren't limited to calling pandas built-ins. Provided that your function takes and returns a DataFrame, you can do pretty much anything you want inside `map_partitions`.
...
[TIP]
====
Dask has special logic to speed up multi-DataFrame joins, so in most cases, rather than doing `a.join(b).join(c).join(d).join(e)`, you will benefit from doing `a.join([b, c, d, e])`. However, if you are performing a left join with a a small dataset, then the first syntax may be more efficient.
====
...
An example is the `split` function from <<wc_dataframe>>. In local pandas, instead of doing `split().explode()`, you could have called `split(expand=true)`. Some of these missing parts can be excellent places where you can get involved and https://github.com/dask/dask/contribute[+++contribute to the Dask project+++] if you are interested.
...
For example, with graphing, it's common to preaggregate the counts or take a random sample and graph the result.
...
In our opinion it's important to be especially careful anytime you use any of these functions when porting your code to use Dask DataFrames, as these could result in difficult-to-track-down correctness issues.
...
Since Dask DataFrames potentially span multiple pandas DataFrames, `.iloc()` (a way to access the positions by index) doesn’t work exactly the same for Dask, since each smaller DataFrame would have its own `.iloc()` value, and Dask does not track the size of each smaller DataFrame. In other words, a global index value is hard for Dask to figure out, since Dask will have to iteratively count through each DataFrame to get to an index. Users should check `.iloc()` on their DataFrame and ensure that indices return the correct values.
