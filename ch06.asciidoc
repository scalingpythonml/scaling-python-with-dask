...
Dask futures are a ((("futures")))((("distributed client", "futures and")))part of Dask's distributed client library, so you will get started by importing it with `from dask.distributed import Client`.
...
Integrating with other async servers (like Tornado):: Although we generally believe that most of the time Dask is not the right solution for the "hot path," there are exceptions, like dynamically computed analytic dashboards.
...
[WARNING]
====
When using Dask in a Jupyter notebook, ((("Jupyter Notebook", "futures")))the notebook may "hold on to" the result of any previous cell, so even if the future is unnamed, it will remain present in Dask. There is a https://oreil.ly/zyy2H[+++discussion on Discourse+++] with more context for those interested.
====

=== Fire-and-Forget

Sometimes you no longer need a future, but you also don't want it to be canceled. This pattern is ((("futures", "fire-and-forget")))((("fire-and-forget", "futures")))called fire-and-forget. This is most useful for things like writing data out, updating a database, or other side effects. If all reference to a future is lost, garbage collection can result in it being canceled. To work around this, Dask has the aptly named `fire_and_forget` method, which allows you to take advantage of this pattern, as shown in <<fire_and_forget_ch06_1686239249531>>, without needing to keep references around.
...
More commonly, eventually, you'll want to ((("futures", "return values", id="ftrrsl")))((("return values", "futures", id="rtvft")))know what the future has computed (or even just if it encountered an error). For futures that are not just side effects, you'll eventually want to get the return value (or error) from the futures. Futures have the blocking method `result`, as shown in <<get_result_ch06_1686239283618>>, which gives you back the value computed in the future or raises the exception from the future.
...
If you've got multiple futures together, say you created them with `map`, you can get the results back as they become available (see <<get_list_results_as_avail_ch06_1686239357228>>). If you can process the results out of order, this can greatly improve your processing time.
...
This time limit can apply either to the entire set or to one at a time. If you want all features finished by a given time, then you need a bit more work, as shown in <<get_any_futures_that_finish_ch06_1686239442955>>.
...
In this (albeit contrived) example, you can see how, by starting the work as soon as possible, the future is completed by the time you get the result, whereas the `dask.delayed` only starts when ((("futures", "return values", startref="ftrrsl")))((("return values", "futures", startref="rtvft")))you get there.

=== Nested Futures

As with `dask.delayed`, you can also ((("futures", "nested")))((("nested futures")))launch futures from inside futures. The syntax is a bit different, as you need to get an instance of the `client` object, which is not serializable, so `dask.distributed` has the special function `get_client` to get the client inside of a distributed function. Once you have the client, you can then launch the future like normal, as shown in <<ex_nested>>.
