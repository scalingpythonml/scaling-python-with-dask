...
Some actions in Dask, like `persist()` on Dask collections, use futures under the hood. You can get the futures of the persisted collection by calling `futures_of`. These futures follow the same lifecycle as the futures that you launch yourself.
...
The string representation of a future will show you where it is in it's lifecycle (e.g. `Future: slow status: cancelled,`). 
...
// PRODUCTION NOTE: Make Distributed Data Structures for Scheduling a call-out box.
[[dds_scheduling]]
