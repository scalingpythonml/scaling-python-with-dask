...
* Request/Response pattern: make a call to a remote service, do some other work (like calling other services), and (later) wait for/block on its result. This can include querying services like databases, remote procedure calls, or even websites.
