...
[TIP]
====
Despite the name, you can use Dask's distributed client locally. Refer to the Distributed section in Chapter 3 for different local deployment types.
====
...
[WARNING]
====
When using Dask in a Jupyter notebook, the notebook may "hold on to" the result of any previous cell, so even if the future is un-named, it will remain present in Dask. There is a https://dask.discourse.group/t/why-is-distributed-leaking-memory-from-unused-futures/959?u=mvashishtha[+++discussion on Discourse+++] with more context for those interested.
====
