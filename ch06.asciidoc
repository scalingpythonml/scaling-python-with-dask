...
[WARNING]
====
When using Dask in a Jupyter notebook, the notebook may "hold on to" the result of any previous cell, so even if the future is unnamed, it will remain present in Dask. There is a https://oreil.ly/zyy2H[+++discussion on Discourse+++] with more context for those interested.
====
